...
var xLineChartValues = ["Lower ω", "Upper ω", "OPA w"];
...
function calc_k_prime(i, j, k, old_k_prime) {
  ...
  for (let x = 0; x < _number_of_alternatives; x++) {
    if (
      parseFloat(expert_criteria_alternatives_rank[i][j][x].rank) < parseFloat(min_rank) &&
      expert_criteria_alternatives_rank[i][j][x].selected == 0 &&
      expert_criteria_alternatives_rank[i][j][x].joft == 0
    ) {
      if (
        // expert_criteria_alternatives_rank[i][j][k].min_rank_joft <=   expert_criteria_alternatives_rank[i][j][x].rank &&
        parseFloat(expert_criteria_alternatives_rank[i][j][k].rank) <
        parseFloat(expert_criteria_alternatives_rank[i][j][x].rank)
      ) {
        min_rank = expert_criteria_alternatives_rank[i][j][x].rank;
        ind_min_rank = x;
        last_min_rank_joft =
  ...
  if (
    ind_min_rank >= 0 &&
    parseFloat(expert_criteria_alternatives_rank[i][j][ind_min_rank].rank) >
    parseFloat(expert_criteria_alternatives_rank[i][j][k].rank)
  ) {
    ind_k_pre_last_joft_for_halate_4 = 1;
  ...
  for (let i = 0; i < _number_of_experts; i++)
    for (let j = 0; j < _number_of_criterias; j++)
      for (
        ...
        if (parseFloat(expert_criteria_alternatives_rank[i][j][x].rank) > parseFloat(max_rank)) {
          max_rank = expert_criteria_alternatives_rank[i][j][x].rank;
          ind_max_rank = x;

          if (
            parseFloat(expert_criteria_alternatives_rank[i][j][ind__C].rank) <
            parseFloat(expert_criteria_alternatives_rank[i][j][ind_max_rank].rank)
          )
            return true;
        }
      }
    ...
    for (let j = 0; j < _number_of_criterias; j++) {
      for (let k = 0; k < _number_of_alternatives; k++) {
        ...
        if (
          ...
          sub_str = sub_str + " ,0";
          arr_temp.push(0);
          arr_mahdodiyatha_A_all[i][j][k].zarib = 0;
        }
        //}
      }
    }
    ...
    if (max_temp) {
      ...
      if (i <= _number_of_criterias) {
        ...
        second_tbl_jadid[i][_number_of_experts + 8] =
          length.toFixed(6) > Tc.toFixed(6) ? "Check" : "Ok";
      } else {
        second_tbl_jadid[i][_number_of_experts + 7] = Ta;
        second_tbl_jadid[i][_number_of_experts + 8] =
          length.toFixed(6) > Ta.toFixed(6) ? "Check" : "Ok";
      }
    }
  }
...
function show_result() {
  ...
  section2kendallxValues.push("Among all criteria");
  ...
  section2localxValues.push("Among all criteria");
...
function func_calc() {
  ...
  if (charts_div1.innerHTML != "") {
    ...
    <a id="sectio1download_chart_e" download="experts.jpg" href="" title="Save">
      <img src="library/save.png" width="20px" />
    </a>
